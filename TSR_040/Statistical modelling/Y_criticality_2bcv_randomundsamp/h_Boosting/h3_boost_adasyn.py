...
print(f"depth tree: {tuned_par[2]}")
print(f"learning rate: {tuned_par[3]}")
print(f"n. trees: {tuned_par[4]}")

# e) Apply ADASYN, Randomundersampler and fit boosting with tuned parameters
...
base_classifier_tuned = DecisionTreeClassifier(max_depth=tuned_par[2],random_state=123)
boost_tuned = AdaBoostClassifier(estimator=base_classifier_tuned, algorithm='SAMME', n_estimators=tuned_par[4], learning_rate=tuned_par[3], random_state=123)
boost_fit = boost_tuned.fit(x_train_resampled, y_train_resampled)
...
plt.title('Variable importance Boosting on ADASYN rebalanced dataset')
