...
# d) Apply SMOTE, tomeklinks and fit tree with tuned parameters
...
tree_fit = tree_tuned.fit(x_train_smote_tomek, y_train_smote_tomek)
...
tree_full_fit = tree_full.fit(x_train_smote, y_train_smote)
...
plot_tree(tree_fit, rounded=True, feature_names=x_train_smote.columns, class_names=['0', '1'], max_depth=max_depth_pruned, fontsize=8)
plt.title('Pruned decision tree on SMOTE rebalanced dataset')
plt.show()