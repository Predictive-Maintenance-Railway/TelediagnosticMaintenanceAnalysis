...
# d) Apply SMOTE, tomeklinks and fit knn with tuned parameters
...
tomek = TomekLinks(sampling_strategy='majority')
x_train_sd_smote_tomek, y_train_smote_tomek = tomek.fit_resample(x_train_sd_smote, y_train_smote)
print(y_train_smote_tomek.value_counts())
knn_tuned = KNeighborsClassifier(n_neighbors=tuned_par[0], weights=tuned_par[1])
knn_fit = knn_tuned.fit(x_train_sd_smote_tomek, y_train_smote_tomek)
