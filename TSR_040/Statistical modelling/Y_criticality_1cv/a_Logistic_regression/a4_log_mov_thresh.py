...
# and the classifier
logistic = LogisticRegression(penalty='l1', solver='liblinear', max_iter=1000, random_state=123)

# c) Grid search with the custom scoring function
grid_search = GridSearchCV(logistic, param_grid, cv=cv, scoring=make_scorer(threshold_f1_scorer, needs_proba=True), n_jobs=-1)
...
for i, is_used in enumerate(non_zero_coefficients):
    if is_used:
        print(f"Feature: {feature_names[i]} (Coefficient: {coefficients[i]:.4f})")
    else:
        print(f"Feature: {feature_names[i]} (Coefficient: {coefficients[i]:.4f})")
print("\n")
print("\nFeatures used in the model:")
