# a) Define random forest classifier
randfor = RandomForestClassifier(n_estimators=500,      # n.of trees grown and aggregated --> 500 as default in R
                                 oob_score=True,
                                 n_jobs=-1,
                                 random_state=123
                                 # criterion=gini by default
                                 # ccp_alpha=0 by default, so no pruning is performed
                                 # max_samples=None by default means each base estimator is trained on a sample of the same size as the original dataset
                                 )
...
# c) Tuning parameters
grid_search = GridSearchCV(randfor, param_grid, cv=cv, scoring=f1_scorer, n_jobs=-1, refit=False)
...
print(f"max n. predictors: {tuned_par}")

# d) Fit rand forest with tuned parameter
randfor_tuned = RandomForestClassifier(n_estimators=500, oob_score=True, n_jobs=-1, random_state=123, max_features=tuned_par)
...
plt.title('Variable importance RandForest on imbalanced dataset')
