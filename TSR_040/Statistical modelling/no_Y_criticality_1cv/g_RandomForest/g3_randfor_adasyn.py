...
param_grid = {
    ...
    'randfor__min_samples_leaf': range(1, 5),
}
...
print(f"max n. predictors: {tuned_par[2]}")
print(f"min size node: {tuned_par[3]}")

# d) Apply ADASYN and fit rand forest with tuned parameters
...
randfor_tuned = RandomForestClassifier(n_estimators=500, oob_score=True, n_jobs=-1, random_state=123, max_features=tuned_par[2], min_samples_leaf=tuned_par[3])
randfor_fit = randfor_tuned.fit(x_train_adasyn, y_train_adasyn)
...
plt.title('Variable importance RandForest on ADASYN rebalanced dataset')